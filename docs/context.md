...
- Jeff fills in a coach's diary on the This Week tab after sessions. One entry per session date, stored only in Neon Postgres and keyed by the date on the server, so every device sees and edits the same entry. Nothing is kept on the device, so saving needs a connection. `tools/diary_pull.py` brings entries into `data/diary.json`; they drive proposed plan changes, never automatic ones. Needs `DATABASE_URL` and `DIARY_PASSPHRASE` set in Vercel.
