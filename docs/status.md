...
- **Phase 2c is built, six of seven tasks.** Both journals, the test sheet, the cross-year charts, the soft delete Jeff asked for and the styling. `docs/superpowers/plans/2026-09-14-phase-2c-writing-screens.md`. The suites stand at `backend/` 249, `core/` 257, `web/` 264.

- **The web app is deployed at `https://teddy-pe-mlfs.vercel.app`**, built from `main`, pointed at `https://teddy-pe-api.fly.dev`. The signed-out check is done and recorded in `docs/decisions.md`: the served JavaScript, CSS and HTML carry nothing about Teddy or his program, the API URL is compiled in correctly, and the security headers arrive. Two things were wrong and both are closed. Deep links all answered 404 because `cleanUrls` disabled the single-page rewrite beside it, fixed on `fix/spa-deep-links`. And `WEB_ORIGIN` was unset on the Fly app, so CORS refused the site and nobody could sign in; Jeff set it on 14 September. Checked against the running API rather than assumed: a preflight from `https://teddy-pe-mlfs.vercel.app` comes back with that exact origin in `access-control-allow-origin`, and one from a foreign origin comes back with no such header at all, so the value is the origin and not a wildcard.
