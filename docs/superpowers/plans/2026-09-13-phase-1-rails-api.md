...
production:
  <<: *default
  # Bracket form rather than ENV.fetch. Rails renders this whole file's ERB as
  # one document before it picks an environment, so a fetch with no default
  # here raises KeyError in development and test as well, and db:create and
  # rspec both die before reaching Postgres. Fly sets this secret at deploy,
  # and a missing one fails loudly at connect time where it belongs.
...
**Files:**
- Replace: `backend/Dockerfile`, `backend/.dockerignore` (both generated by `rails new`; this task overwrites them)
- Create: `backend/fly.toml`, `backend/config/initializers/okcomputer.rb`
- Modify: `backend/config/puma.rb`, `backend/.github/workflows/ci.yml`
- Delete: `backend/bin/thrust`

`config/environments/production.rb` needs no change. Rails 8 logs to stdout when `RAILS_LOG_TO_STDOUT` is set, which `fly.toml` sets, HTTPS is forced at the Fly edge, and `config.hosts` protection is development-only. Do not invent an edit to it.
...
- [ ] **Step 4b: Clear out the generated scaffolding this app does not use**

`rails new` left two things behind that the Task 1 review caught.

Delete `backend/bin/thrust`. The generated Dockerfile ran the server through Thruster, but `thruster` was never in the closed Gemfile, so the script loads a gem that is not installed. Step 2 already replaced that Dockerfile with a plain `rails server` CMD, which leaves `bin/thrust` as a dead file pointing at a missing gem.

Then give CI a job that actually runs the suite. The generated `backend/.github/workflows/ci.yml` has only `scan_ruby` (Brakeman) and `lint` (Rubocop), so nothing runs `rspec`. There is no staging environment by design, which makes the test suite the only thing standing between a bad edit and Teddy, so it has to run on every push. Add this job alongside the existing two:
  ...
  test:
    ...
    steps:
      ...
      - name: Run the suite
        ...
        env:
          RAILS_ENV: test
          TEST_DATABASE_URL: postgresql://postgres:postgres@localhost:5432/teddy_pe_test
        run: |
          bundle exec rails db:create db:schema:load
          bundle exec rspec
```

The content spec needs no database, so it stays green even if the Postgres service is slow to come up.
